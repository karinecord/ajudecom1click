class Backoffice::DisappearedsController < ApplicationController
  ...
  def create
    ...
    @disappeared.created_by = current_admin

    respond_to do |format|
      if @disappeared.save
        format.html { redirect_to backoffice_relative_disappeared_path(@relative, @disappeared), notice: t('.success') }
  ...
  def update
    respond_to do |format|
      if @disappeared.update(disappeared_params)
        format.html { redirect_to backoffice_relative_disappeared_path(@relative, @disappeared), notice: t('.updated' ) }
  ...
  def destroy
    @disappeared.destroy
    respond_to do |format|
      format.html { redirect_to backoffice_relative_disappeareds_path(@relative), notice: t('.destroyed')}
  ...
  def set_relative
    ...
    def disappeared_params
      params.require(:disappeared)
          .permit(:name, :nickname,:email, :password,  :notification_face, :id_number, :cpf, :drivers_license, :date_of_birth, :gender, :nationality, :state, :marital_status, :address, :telephone, :relatives_id, :cell_phone, :photo)
    end
end
