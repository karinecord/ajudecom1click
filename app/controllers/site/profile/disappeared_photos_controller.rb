class Site::Profile::DisappearedPhotosController < Site::ProfileController
  ...
  def create
    if photo_params[:disappeared_photo]
      if @disappeared.photos.length >= MAX_PHOTOS_ALLOWED
        ...
        return redirect_to site_profile_relative_disappeared_path(@disappeared.relative, @disappeared)
      end
      ...
      redirect_to site_profile_relative_disappeared_path(@disappeared.relative, @disappeared), notice: t('.success')
    else
      flash[:alert] = 'Vc precisa seleciona uma foto'
      redirect_to site_profile_relative_disappeared_path(@disappeared.relative, @disappeared)
    end
  end
  ...
  def destroy
    ...
    redirect_to site_profile_relative_disappeared_path(@disappeared.relative, @disappeared), notice: t('.deleted')
  end
